...
class InitialViewController: UIViewController,UITableViewDelegate, UITableViewDataSource {
    ...
    private func initForStatisticPanel() {
        ...
        self.initialPanel?.snp.makeConstraints{ (make) -> Void in
            make.edges.equalTo(UIEdgeInsets(top: 0, left: 0, bottom: screenFrame.height - screenFrame.width * 0.8, right: 0))
    ...
    func drawPlot() {
        ...
        let line = LineChartDataSet(entries: dataEntrySet, label: NSLocalizedString("Cost", comment: "Cost"))
        line.colors = [NSUIColor.cyan]
        line.circleColors = [NSUIColor.cyan]
        let line2 = LineChartDataSet(entries: dataEntrySet2, label: NSLocalizedString("Earn", comment: "Earn"))
    ...
    private func initForTableView() {
        ...
        self.initialtable?.snp.makeConstraints{ (make) -> Void in
            make.edges.equalTo(UIEdgeInsets(top: screenFrame.width * 0.8, left: 0, bottom: 0, right: 0))
        }
    }
...
//        }
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let deleteAction = UITableViewRowAction(style: UITableViewRowAction.Style.default, title: NSLocalizedString("Delete", comment: "Delete String")) { (action , index) -> Void in
